...
def main() -> int:
    ...
    graph = gm.adjacency((e["source"], e["target"])
                         for e in read("edges_person_person.csv"))
    giant = gm.giant_component(graph)
    betweenness = gm.betweenness(giant)
    closeness = gm.closeness(giant)
    clustering = gm.clustering(giant)
    degree = {node: len(neighbours) for node, neighbours in giant.items()}
    ...
    sample = []
    # sorted(), not the graph's own order. The regression is invariant to row
    # order, but the permutation test below shuffles this list under a fixed
    # seed, so a different order gives a different sequence of draws and a
    # slightly different Monte Carlo p. Iterating the graph made that p depend
    # on how the graph happened to be built rather than on the data.
